...
def test_json_methods():
    ...
    epoch_stats = dict()
    epoch_stats["epoch"] = 0
    epoch_stats['total_d_train_loss_mean'] = 0.1
    epoch_stats['total_d_train_loss_std'] = 0.1
    epoch_stats['total_g_train_loss_mean'] = 0.1
    epoch_stats['total_g_train_loss_std'] = 0.1
    epoch_stats['total_d_val_loss_mean'] = 0.1
    epoch_stats['total_d_val_loss_std'] = 0.1
    epoch_stats['total_g_val_loss_mean'] = 0.1
    epoch_stats['total_g_val_loss_std'] = 0

    # if self.args.use_pretrained_classifier_for_quantitative_results:
    #     epoch_stats['total_train_real_accuracy_mean'] = np.mean(train_real_accuracy)
    #     epoch_stats['total_train_real_accuracy_std'] = np.std(train_real_accuracy)
    #     epoch_stats['total_train_fake_accuracy_mean'] = np.mean(train_fake_accuracy)
    #     epoch_stats['total_train_fake_accuracy_std'] = np.std(train_fake_accuracy)
    #     epoch_stats['total_val_real_accuracy_mean'] = np.mean(val_real_accuracy)
    #     epoch_stats['total_val_real_accuracy_std'] = np.std(val_real_accuracy)
    #     epoch_stats['total_val_fake_accuracy_mean'] = np.mean(val_fake_accuracy)
    #     epoch_stats['total_val_fake_accuracy_std'] = np.std(val_fake_accuracy)
    _, log_path, _ = build_experiment_folder(experiment_name=args.experiment_title)
    create_json_experiment_log(experiment_log_dir=log_path, args=args)
    for i in range(100):
        change_json_log_experiment_status(experiment_log_dir=log_path, experiment_status="Running")
        update_json_experiment_log_epoch_stats(epoch_stats=epoch_stats, experiment_log_dir=log_path)
        print(i)

def unzip_file(filepath_pack, filepath_to_store):
    print("Unzip file: {} to path: {}".format(filepath_pack, filepath_to_store))
    os.system("tar -I pbzip2 -xf {} -C {}".format(filepath_pack, filepath_to_store))
#test_json_methods()